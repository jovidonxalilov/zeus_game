...
class GameBloc extends Bloc<GameEvent, bloc_state.GameBlocState> {
  ...
  Future<void> _onInitializeGame(
    ...
    try {
      ...
      // Try to play background music (don't crash if it fails)
      try {
        await _audioManager.playMusic('music/game.mp3');
      } catch (e) {
  ...
  void _onResumeGame(
    ...
    // Try to play music (don't crash if it fails)
    try {
      _audioManager.playMusic('music/game.mp3');
    } catch (e) {
      print('Warning: Could not resume music: $e');
    }
  }
